	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			if Config.Uniforms[job].male then
				...
				exports['mythic_notify']:DoHudText('error', _U('no_outfit'))
			end
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						exports['mythic_notify']:DoHudText('error', _U('service_out'))
					end
				end, 'police')
			end
		end
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							exports['mythic_notify']:DoHudText('error', _U('service_max', inServiceCount, maxInService))
						else
							serviceOk = true
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
							TriggerEvent('esx_policejob:updateBlip')
							exports['mythic_notify']:DoHudText('error', _U('service_in'))
						end
					end, 'police')
...
function StoreNearbyVehicle(playerCoords)
	...
	if #vehicles > 0 then
		...
		exports['mythic_notify']:DoHudText('error', _U('garage_store_nearby'))
		return
	end
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	...
	if found then
		...
		exports['mythic_notify']:DoHudText('error', _U('vehicle_blocked'))
		return false
	end
end
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_policejob:buyJobVehicle', function (bought)
					if bought then
						...
						exports['mythic_notify']:DoHudText('error', _U('vehicleshop_money'))
						menu2.close()
					end
		...
		if data.current.value == 'citizen_interaction' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					exports['mythic_notify']:DoHudText('error', _U('no_players_nearby'))
				end
			end, function(data2, menu2)
				menu2.close()
			end)
						...
						Citizen.CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									exports['mythic_notify']:DoHudText('error', _U('impound_canceled_moved'))
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			exports['mythic_notify']:DoHudText('error', _U('search_database_error_invalid'))
		else
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					exports['mythic_notify']:DoHudText('error', _U('search_database_found', owner))
				else
					exports['mythic_notify']:DoHudText('error', _U('search_database_error_not_found'))
				end
			end, data.value)
			menu.close()
		end
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			exports['mythic_notify']:DoHudText('error', _U('licence_you_revoked', data.current.label, targetName))
	...
	ESX.TriggerServerCallback('esx_policejob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					exports['mythic_notify']:DoHudText('error', _U('quantity_invalid'))
				else
					menu2.close()
					menu.close()
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_armory' then
					...
					elseif playerInService then
						...
						exports['mythic_notify']:DoHudText('error', _U('service_not'))
					end
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.handcuffTimer, function()
		exports['mythic_notify']:DoHudText('error', _U('unrestrained_timer'))
		TriggerEvent('esx_policejob:unrestrain')
...
function ImpoundVehicle(vehicle)
	...
	ESX.Game.DeleteVehicle(vehicle)
	exports['mythic_notify']:DoHudText('success', _U('impound_successful'))
	currentTask.busy = false
end
