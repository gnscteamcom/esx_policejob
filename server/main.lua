...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('got_confiscated', amount, sourceItem.label, sourceXPlayer.name)})
			end
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = _U('quantity_invalid')})
		end
	...
	elseif itemType == 'item_account' then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('got_confiscated_account', amount, itemName, sourceXPlayer.name)})
	...
	elseif itemType == 'item_weapon' then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('got_confiscated_weapon', ESX.GetWeaponLabel(itemName), amount, sourceXPlayer.name)})
	end
end)
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = _U('have_deposited', count, inventoryItem.label)})
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = _U('quantity_invalid')})
		end
	end)
